...
if (!empty($videos_inprogress))
{
    ...
    foreach ($videos as $item):
        ...
        $s_f = (substr($item->date_finish, 0, 4) == '0000') ? '': $item->date_finish;
...
//                              .'</div>',
                              ...
                              (count($translators)>1?implode(", ", '<small>'.$translators.'</small>')." <br/>".anchor('languages/'.$team->langcode.'/videos/register_function/'.$item->id.'/'.FUNCTION_TRANSLATE.'/on_hold','I\'m going in!', array('class' => 'tiny button', 'onclick' => 'open_forum(\''.$item->forum_thread.'\')')):
                                                     '<small>'.$translators[0].'</small>'." <br/>".anchor('languages/'.$team->langcode.'/videos/register_function/'.$item->id.'/'.FUNCTION_TRANSLATE.'/on_hold','I\'m going in!', array('class' => 'tiny button', 'onclick' => 'open_forum(\''.$item->forum_thread.'\')'))):
                              ...
                              (count($proofreaders)>1?implode(", ", '<small>'.$proofreaders).'</small>'." <br/>".anchor('languages/'.$team->langcode.'/videos/register_function/'.$item->id.'/'.FUNCTION_PROOFREAD.'/on_hold','I\'m going in!', array('class' => 'tiny button', 'onclick' => 'open_forum(\''.$item->forum_thread.'\')')):
                                                      '<small>'.$proofreaders[0].'</small>'." <br/>".anchor('languages/'.$team->langcode.'/videos/register_function/'.$item->id.'/'.FUNCTION_PROOFREAD.'/on_hold','I\'m going in!', array('class' => 'tiny button', 'onclick' => 'open_forum(\''.$item->forum_thread.'\')'))):
...
{
    echo '<div class="alert-box ">There is no videos on hold!</div>';
    echo '<div><img src="'.base_url().'/img/good.jpg"></div>';
}
