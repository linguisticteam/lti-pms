...
if (!empty($videos_inprogress))
{
    ...
    foreach ($videos as $item):
        ...
        $this->table->add_row($i, 
                              ...
                              (count($translators)>1?implode(", ", $translators)." <br/>".anchor('languages/'.$team->shortname.'/videos/register_function/'.$item->id.'/'.FUNCTION_TRANSLATE.'/on_hold','I did it!'):
                                                     $translators[0]." <br/>".anchor('languages/'.$team->shortname.'/videos/register_function/'.$item->id.'/'.FUNCTION_TRANSLATE.'/on_hold','I did it!')):
                              (count($translators)>1?implode(", ", $translators):$translators[0]),
                              ($member_role >= MEMBER_ROLE_TRANSLATOR)?
                              (count($proofreaders)>1?implode(", ", $proofreaders)." <br/>".anchor('languages/'.$team->shortname.'/videos/register_function/'.$item->id.'/'.FUNCTION_PROOFREAD.'/on_hold','I did it!'):
                                                      $proofreaders[0]." <br/>".anchor('languages/'.$team->shortname.'/videos/register_function/'.$item->id.'/'.FUNCTION_PROOFREAD.'/on_hold','I did it!')):
