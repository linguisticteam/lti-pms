...
//                              .'</div>',
                              ...
                              (count($translators)>1?implode(", ", '<small>'.$translators.'</small>')." <br/>".anchor('languages/'.$team->langcode.'/videos/register_function/'.$item->id.'/'.FUNCTION_TRANSLATE.'/ready_to_post','I\'m going in!', array('class' => 'tiny button', 'onclick' => 'open_forum(\''.$item->forum_thread.'\')')):
                                                     '<small>'.$translators[0].'</small>'." <br/>".anchor('languages/'.$team->langcode.'/videos/register_function/'.$item->id.'/'.FUNCTION_TRANSLATE.'/ready_to_post','I\'m going in!', array('class' => 'tiny button', 'onclick' => 'open_forum(\''.$item->forum_thread.'\')'))):
                              ...
                              (count($proofreaders)>1?implode(", ", '<small>'.$proofreaders.'</small>')." <br/>".anchor('languages/'.$team->langcode.'/videos/register_function/'.$item->id.'/'.FUNCTION_PROOFREAD.'/ready_to_post','I\'m going in!', array('class' => 'tiny button', 'onclick' => 'open_forum(\''.$item->forum_thread.'\')')):
                                                     '<small>'.$proofreaders[0].'</small>'." <br/>".anchor('languages/'.$team->langcode.'/videos/register_function/'.$item->id.'/'.FUNCTION_PROOFREAD.'/ready_to_post','I\'m going in!', array('class' => 'tiny button', 'onclick' => 'open_forum(\''.$item->forum_thread.'\')'))):
...
<script>
    ...
    {
        window.location = "<?php echo base_url() . 'languages/' . $team->langcode; ?>/videos/go_to_stage_table/" + id + "/" + state + "/-1/ready_to_post";
        return false;
    }
    ...
    {
        window.location = "<?php echo base_url() . 'languages/' . $team->langcode; ?>/videos/release_video_table/" + id + "/ready_to_post";
        return false;
    }
